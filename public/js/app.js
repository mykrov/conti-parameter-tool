...
// ==========================================
// 6. TABLA: facturas (cabecera + detalle)
// ==========================================
function setupEventListenersFacturas() {
...
window.inspectFactura = function(id) {
  ...
  dom.modalTitle.textContent = `Factura: ${factura.documento || `#${id}`}`;
  dom.modalSubtitle.textContent = `Tabla: factura_cabecera • Detalle: ${factura.total_items || 0} líneas`;
  dom.modalJsonContent.textContent = JSON.stringify(factura, null, 2);
  dom.detailModal.style.display = 'flex';
};
...
function exportFacturasJson() {
  dom.modalTitle.textContent = `Tabla: facturas (${state.facturas.data.length} registros)`;
  dom.modalSubtitle.textContent = `factura_cabecera + factura_detalle • pos_contifico @ MySQL 5.7`;
  dom.modalJsonContent.textContent = JSON.stringify(state.facturas.data, null, 2);
  dom.detailModal.style.display = 'flex';
}
